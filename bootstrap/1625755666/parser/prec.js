...
};module.exports = (async function(){return(await pn.cons($List,[await pn.cons($List,[`,`]),await pn.cons($List,[`=`]),await pn.cons($List,[`->`]),await pn.cons($List,[`||`,`&&`]),await pn.cons($List,[`>`,`<`,`<=`,`>=`,`!=`,`==`]),await pn.cons($List,[`+`,`-`]),await pn.cons($List,[`*`,`/`,`%`]),await pn.cons($List,[`**`]),await pn.cons($List,[`.`])]));})();